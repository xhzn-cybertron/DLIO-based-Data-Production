...
dlio::OdomNode::OdomNode() : Node("dlio_odom_node") {
  ...
  // this->pose_outFile.open("/home/ros-monitor/a_paper_exp_ws/pose.txt",std::ofstream::out);
  std::string pose_yaml_file = "/home/ros-monitor/a_paper_exp_ws/pose.yaml";
  this->yaml = fopen(pose_yaml_file.c_str(), "w");
  ...
  this->submap_cloud = pcl::make_shared<pcl::PointCloud<PointType>>();
...
dlio::OdomNode::~OdomNode() {  fflush(yaml); fclose(yaml);}
...
void dlio::OdomNode::getParams() {
  ...
  dlio::declare_param(this, "~dlio/frames/baselink", this->baselink_frame,
                      "base_link");
  dlio::declare_param(this, "~dlio/frames/lidar", this->lidar_frame,
                      "laser_link");
  ...
  // Keyframe Threshold
  // dlio::declare_param(this, "~dlio/odom/keyframe/threshD",
  //                     this->keyframe_thresh_dist_, 0.5);
  // dlio::declare_param(this, "~dlio/odom/keyframe/threshR",
  //                     this->keyframe_thresh_rot_, 5.0);
  dlio::declare_param(this, "~dlio/odom/keyframe/threshD",
                      this->keyframe_thresh_dist_, 1.0);
  dlio::declare_param(this, "~dlio/odom/keyframe/threshR",
                      this->keyframe_thresh_rot_, 45.0);

  // Submap
  dlio::declare_param(this, "~dlio/odom/submap/keyframe/knn", this->submap_knn_,
                      20);
  dlio::declare_param(this, "~dlio/odom/submap/keyframe/kcv", this->submap_kcv_,
                      8);
  dlio::declare_param(this, "~dlio/odom/submap/keyframe/kcc", this->submap_kcc_,
                      8);

  // Dense map resolution
  dlio::declare_param(this, "~dlio/map/dense/filtered",
                      this->densemap_filtered_, true);

  // Wait until movement to publish map
  dlio::declare_param(this, "~dlio/map/waitUntilMove", this->wait_until_move_,
                      false);

  // Crop Box Filter
  dlio::declare_param(this, "~dlio/odom/preprocessing/cropBoxFilter/size",
                      this->crop_size_, 1.0);

  // Voxel Grid Filter
  dlio::declare_param(this, "~dlio/pointcloud/voxelize", this->vf_use_, true);
  // dlio::declare_param(this, "~dlio/odom/preprocessing/voxelFilter/res",
  //                     this->vf_res_, 0.25);
  dlio::declare_param(this, "~dlio/odom/preprocessing/voxelFilter/res",
                      this->vf_res_, 0.1);
  ...
  // Extrinsics
  std::vector<double> t_default1{1.29, 0.1, 1.5};
  std::vector<double> R_default1{1., 0., 0., 0., 1., 0., 0., 0., 1.};

  // center of gravity to imu
  std::vector<double> baselink2imu_t, baselink2imu_R;
  dlio::declare_param(this, "~dlio/extrinsics/baselink2imu/t", baselink2imu_t,
                      t_default1);
  dlio::declare_param(this, "~dlio/extrinsics/baselink2imu/R", baselink2imu_R,
                      R_default1);
  ...
  std::vector<double> t_default2{1.29, 0., 2.2};
  std::vector<double> R_default2{1., 0., 0., 0., 1., 0., 0., 0., 1.};
  std::vector<double> baselink2lidar_t, baselink2lidar_R;
  dlio::declare_param(this, "~dlio/extrinsics/baseink2lidar/t",
                      baselink2lidar_t, t_default2);
  dlio::declare_param(this, "~dlio/extrinsics/baselink2lidar/R",
                      baselink2lidar_R, R_default2);

  // this->extrinsics.baselink2lidar.t =
  //   Eigen::Vector3f(baselink2lidar_t[0], baselink2lidar_t[1],
  //   baselink2lidar_t[2]);
  this->extrinsics.baselink2lidar.t =
  ...
  this->extrinsics.baselink2lidar_T.block(0, 3, 3, 3) =
      this->extrinsics.baselink2lidar.t;
  this->extrinsics.baselink2lidar_T.block(0, 0, 2, 2) =
      this->extrinsics.baselink2lidar.R;

  // IMU
  dlio::declare_param(this, "~dlio/odom/imu/calibration/accel",
                      this->calibrate_accel_, true);
  dlio::declare_param(this, "~dlio/odom/imu/calibration/gyro",
                      this->calibrate_gyro_, true);
  dlio::declare_param(this, "~dlio/odom/imu/calibration/time",
                      this->imu_calib_time_, 3.0);
  dlio::declare_param(this, "~dlio/odom/imu/bufferSize", this->imu_buffer_size_,
                      2000);
  ...
  dlio::declare_param(this, "~dlio/odom/imu/approximateGravity",
                      this->gravity_align_, false);
  dlio::declare_param(this, "~dlio/imu/calibration", this->imu_calibrate_,
                      true);
  dlio::declare_param(this, "~dlio/imu/intrinsics/accel/bias", prior_accel_bias,
                      accel_default);
  dlio::declare_param(this, "~dlio/imu/intrinsics/gyro/bias", prior_gyro_bias,
                      gyro_default);
  ...
  dlio::declare_param(this, "~dlio/imu/intrinsics/accel/sm", imu_sm,
                      imu_sm_default);
  ...
  // GICP
  dlio::declare_param(this, "~dlio/odom/gicp/minNumPoints",
                      this->gicp_min_num_points_, 64);
  dlio::declare_param(this, "~dlio/odom/gicp/kCorrespondences",
                      this->gicp_k_correspondences_, 20);
  dlio::declare_param(this, "~dlio/odom/gicp/maxCorrespondenceDistance",
                      this->gicp_max_corr_dist_, 150.0);
  dlio::declare_param(this, "~dlio/odom/gicp/maxIterations",
                      this->gicp_max_iter_, 48);
  dlio::declare_param(this, "~dlio/odom/gicp/transformationEpsilon",
                      this->gicp_transformation_ep_, 0.001);
  dlio::declare_param(this, "~dlio/odom/gicp/rotationEpsilon",
                      this->gicp_rotation_ep_, 0.001);
  dlio::declare_param(this, "~dlio/odom/gicp/initLambdaFactor",
                      this->gicp_init_lambda_factor_, 1e-9);

  // Geometric Observer
  dlio::declare_param(this, "~dlio/odom/geo/Kp", this->geo_Kp_, 1.0);
  dlio::declare_param(this, "~dlio/odom/geo/Kv", this->geo_Kv_, 2.25);
  dlio::declare_param(this, "~dlio/odom/geo/Kq", this->geo_Kq_, 0.8);
  dlio::declare_param(this, "~dlio/odom/geo/Kab", this->geo_Kab_, 1.25);
  dlio::declare_param(this, "~dlio/odom/geo/Kgb", this->geo_Kgb_, 1.0);
  dlio::declare_param(this, "~dlio/odom/geo/abias_max", this->geo_abias_max_,
                      1.5);
  dlio::declare_param(this, "~dlio/odom/geo/gbias_max", this->geo_gbias_max_,
                      0.8);
}
...
void dlio::OdomNode::deskewPointcloud() {
  ...
  double sweep_ref_time =
      static_cast<double>(rclcpp::Time(this->scan_header_stamp).seconds());
  //   std::cout << "rclcpp::Time(this->scan_header_stamp).seconds() * 10e9:" <<
  //   rclcpp::Time(this->scan_header_stamp).seconds() * 10e9 << std::endl;
  // std::cout << "rclcpp::Time(this->scan_header_stamp).seconds():" <<
  // rclcpp::Time(this->scan_header_stamp).seconds() << std::endl;
  ...
  if (this->sensor == dlio::SensorType::OUSTER) {
    ...
    extract_point_time =
        [&sweep_ref_time](boost::range::index_value<PointType&, long> pt) {
          ...
          // std::cout << "sweep_ref_time - pt.value().time * 10e6:" <<
          // sweep_ref_time - pt.value().time * 10e6 << std::endl;
        };
  ...
  // std::cout << "this->imu_buffer.back().stamp2:" <<
  // this->imu_buffer.back().stamp  << std::endl; don't process scans until IMU
  // data is present
  if (!this->first_valid_scan) {
    // std::cout << "this->imu_buffer.back().stamp2:" <<
    // this->imu_buffer.back().stamp / 1000000000 << std::endl;
    //   std::cout << "this->scan_stamp1:" << this->scan_stamp << std::endl;
    if (this->imu_buffer.empty() ||
        this->scan_stamp <= (this->imu_buffer.back().stamp)) {
      return;
  ...
  std::vector<Eigen::Matrix4f, Eigen::aligned_allocator<Eigen::Matrix4f>>
      frames;
  // std::cout << "this->prev_scan_stamp:" << this->prev_scan_stamp <<
  // std::endl;
...
void dlio::OdomNode::callbackPointCloud(
  ...
  std::thread(&dlio::OdomNode::savePose, this).detach();
  std::thread(&dlio::OdomNode::savePCD, this).detach();
  // this->savePose(); 
  // this->savePCD();
...
void dlio::OdomNode::callbackImu(
  ...
  double imu_stamp_secs =
      static_cast<double>(rclcpp::Time(imu->header.stamp).seconds());
...
// void dlio::OdomNode::stateToMatrix(State state, Eigen::Matrix4f &pose2save)
// {
//     pose2save.setIdentity();
//     pose2save.block<3,3>(0,0) = state.q.toRotationMatrix();
//     pose2save(0,3) = state.p.x(); pose2save(1,3) = state.p.y(); pose2save(2,3) = state.p.z();
//     pose2save = pose2save * this->extrinsics.baselink2lidar_T;
// }

// void dlio::OdomNode::matrixToState(const Eigen::Matrix4f &pose2save, State &state)
// {
//     state.q = Eigen::Quaternionf(pose2save.block<3,3>(0,0));
//     state.q.normalize();
//     state.p.x() = pose2save(0,3); state.p.y() = pose2save(1,3); state.p.z() = pose2save(2,3);
// }

void dlio::OdomNode::savePose() 
{
    Eigen::Matrix4f poseLidar; 
    poseLidar.setIdentity();
    // this->geo.mtx.lock();
    State odom = this->state;
    // this->geo.mtx.unlock();
    poseLidar.block<3,3>(0,0) = odom.q.toRotationMatrix();
    poseLidar(0,3) = odom.p.x(); poseLidar(1,3) = odom.p.y(); poseLidar(2,3) = odom.p.z();
    // poseLidar = poseLidar * this->extrinsics.baselink2lidar_T;
    // std::cout << this->extrinsics.baselink2lidar_T << std::endl;

    State stateToSave; 
    stateToSave.q = Eigen::Quaternionf(poseLidar.block<3,3>(0,0));
    stateToSave.q.normalize();
    stateToSave.p.x() = poseLidar(0,3); stateToSave.p.y() = poseLidar(1,3); stateToSave.p.z() = poseLidar(2,3);

    // std::cout << to_string_with_precision(this->scan_stamp,18) << std::endl;
    // // std::cout<<std::setprecision(18) << scan_stamp << " "<<std::setprecision(6) 
    // // << stateToSave.p.x() << " " << stateToSave.p.y() << " " << stateToSave.p.z() << " "
    // // << stateToSave.q.x() << " " << stateToSave.q.y() << " " << stateToSave.q.z() << " " << stateToSave.q.w() << "\n";
    
    // std::ofstream fout(map_yaml_file);
    fprintf(yaml,
          "%.8f %.8f %.8f %.8f %.8f %.8f %.8f %.8f\n",
          this->scan_stamp, stateToSave.p.x(), stateToSave.p.y(), stateToSave.p.z(),
          stateToSave.q.x(), stateToSave.q.y(), stateToSave.q.z(), stateToSave.q.w());
    fflush(yaml);
}

void dlio::OdomNode::savePCD() 
{
    Eigen::Matrix4f T_inverse = this->T.inverse();
    // T_inverse.normalize();
    pcl::PointCloud<PointType>::Ptr pc_original_lidar(
        new pcl::PointCloud<PointType>(*this->deskewed_scan));
    // pc_original_lidar->reserve(this->deskewed_scan->size());
    pcl::transformPointCloud(*pc_original_lidar, *pc_original_lidar, T_inverse);

    static size_t pcd_count;
    std::stringstream ss;
    ss << pcd_count++;

    std::string pcd_file = this->pcd_path + ss.str() + ".pcd";
    // std::cout << pcd_file << std::endl;
    pcl::io::savePCDFileBinary(pcd_file, *pc_original_lidar);
}
...
void dlio::OdomNode::computeConvexHull() {
  ...
  pcl::PointIndices::Ptr convex_hull_point_idx =
      pcl::make_shared<pcl::PointIndices>();
